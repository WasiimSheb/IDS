...
def log_to_file(log_file, message):
    ...
    """
    # Make sure the message is being written to the provided log file
    if isinstance(log_file, str):
        with open(log_file, 'a') as f:  # Open in append mode
            f.write(message.encode("cp1252", errors="replace").decode("cp1252") + "\n")
    else:
    ...
    if ICMP in packet:
        ...
        if is_internal_ip(src_ip) and is_external_ip(dst_ip):
            ...
            if packet_count > icmp_packet_threshold:
                ...
                log_attack("ICMP Data Exfiltration", f"Suspicious ICMP traffic: {packet_count} packets from {src_ip} to {dst_ip} in {icmp_time_window} seconds.")
            if total_data_sent > icmp_total_data_threshold:
                ...
                log_attack("ICMP Data Exfiltration", f"Suspicious ICMP traffic: {total_data_sent} bytes from {src_ip} to {dst_ip} in {icmp_time_window} seconds.")
            if payload_size > icmp_payload_threshold:
                ...
                log_attack("ICMP Data Exfiltration", f"Suspicious ICMP traffic: Large packet ({payload_size} bytes) from {src_ip} to {dst_ip}.")
...
def detect_excessive_dns_queries(packet, log_file, dns_query_threshold=100, dns_time_window=60):
    ...
    if DNS in packet and packet[DNS].qd:
        ...
        log_attack("DNS Query", f"Detected DNS Query: {query_raw} with encoding {encoding}")
        ...
        try:
            ...
            log_attack("DNS Query", f"Decoded DNS Query: {query}")
            ...
            log_attack("DNS Query", f"Replaced non-ASCII DNS Query: {query}")
        ...
        if len(dns_tracker[src_ip]) > dns_query_threshold:
            ...
            log_attack("DNS Query", f"Excessive DNS Queries Detected: {src_ip} made {len(dns_tracker[src_ip])} DNS queries in {dns_time_window} seconds.")
...
def detect_file_transfer_protocols(flow_key, log_file):
    ...
    if protocol == "TCP" and (dst_port in [21, 22] or src_port in [21, 22]):
        ...
        log_attack("File Transfer", f"Unauthorized File Transfer Detected: {src_ip} to {dst_ip}. File transfer protocol on port {dst_port if dst_port in [21, 22] else src_port}")
    ...
    if TCP in packet and packet[TCP].dport == 80:  # HTTP typically runs over TCP port 80
        ...
        if "User-Agent" in http_data:
            ...
            if len(user_agent_value) > user_agent_length_threshold:
                ...
                log_attack("HTTP Covert Channel", f"HTTP Covert Channel Detected in User-Agent: {user_agent_value}")
...
def detect_syn_flood(packet, log_file, syn_flood_threshold=100):
    ...
    if TCP in packet and (packet[TCP].flags & 0x12 == 0x02):  # SYN set, ACK not set
        ...
        if syn_flood_tracker[src_ip] > syn_flood_threshold:
            ...
            log_attack("SYN Flood", f"SYN flood detected from {src_ip}")
...
def detect_dns_exfiltration(packet, log_file, query_length_threshold=50):
    ...
    if DNS in packet and packet[DNS].qd:  # DNS query
        ...
        try:
            ...
            log_attack("DNS Exfiltration", f"Decoding issue with {encoding}, replaced invalid characters in query: {query}")
        ...
        if len(query) > query_length_threshold:
            ...
            log_attack("DNS Exfiltration", f"Possible DNS exfiltration in query {query}")
...
def detect_port_scan(src_ip, dst_port, packet_time, log_file, port_scan_threshold=10):
    ...
    if len(scanning_ips[src_ip]["ports"]) > port_scan_threshold:
        ...
        log_attack("Port Scan", f"Port scanning detected from {src_ip}")
...
def detect_traffic_anomalies(flow_key, log_file):
    ...
    if len(traffic_history[src_ip]) > 1:
        ...
        if current_traffic > 2 * avg_traffic:
            ...
            log_attack("Traffic Anomaly", f"Traffic Anomaly Detected: {src_ip} shows sudden traffic spike. Current: {current_traffic}, Avg: {avg_traffic}")
